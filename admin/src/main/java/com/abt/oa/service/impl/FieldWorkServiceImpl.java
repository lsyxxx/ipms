...
public class FieldWorkServiceImpl implements FieldWorkService {
    ...
    private static final String[] FIELD_WORK_SIMPLE_EXPORT_HEADERS = {
            "工号", "姓名", "考勤日期", "补助项目", "补助合计(元)", "井号", "项目", "作业内容",
            "审核人", "审核日期", "审批状态", "审核结果", "审批时间"
    };
...
//                .withIgnorePaths("enable", "sort");
    ...
    @Override
    public void createExportExcel(FieldWorkRequestForm form, OutputStream outputStream) {
        Assert.notNull(form, "导出条件不能为空");
        int currentPage = 1;
        int pageSize = 100;
        ...
        try {
            ...
            int totalCount = 0;
            while (true) {
                final Page<FieldWork> page = findAllRecords(form);
                final List<FieldWork> exportData = page.getContent().stream()
                        .filter(Objects::nonNull)
                        .toList();
                if (exportData.isEmpty()) {
                    break;
            ...
            applyFieldWorkExportColumnWidth(cells);
            workbook.save(outputStream, SaveFormat.XLSX);
            log.info("field work export excel generated by page, record size: {}", totalCount);
        } catch (Exception e) {
            log.error("分页生成野外记录导出excel失败", e);
            throw new BusinessException("生成野外记录导出excel失败:" + e.getMessage());
        }
    }

    @Override
    public void createSimpleExportExcel(FieldWorkRequestForm form, OutputStream outputStream) {
        Assert.notNull(form, "导出条件不能为空");
        int currentPage = 1;
        int pageSize = 100;
        form.setPage(currentPage);
        ...
        try {
            ...
            writeFieldWorkSimpleExportHeader(cells, headerStyle);
            int rowIndex = 1;
            int totalCount = 0;
            while (true) {
                final Page<FieldWork> page = findAllRecords(form);
                ...
                for (FieldWork fw : exportData) {
                    writeFieldWorkSimpleExportRow(cells, rowIndex, fw, resolveFieldWorkAllowanceNames(fw), textStyle, dateStyle, dateTimeStyle, amountStyle);
                    rowIndex++;
                }
                totalCount += exportData.size();
                if (!page.hasNext()) {
            ...
            applyFieldWorkSimpleExportColumnWidth(cells);
            workbook.save(outputStream, SaveFormat.XLSX);
            log.info("field work simple export excel generated by page, record size: {}", totalCount);
        } catch (Exception e) {
            log.error("分页生成野外记录主表excel失败", e);
            throw new BusinessException("生成野外记录主表excel失败:" + e.getMessage());
        }
    }
    ...
    private void writeFieldWorkSimpleExportHeader(Cells cells, Style headerStyle) {
        for (int i = 0; i < FIELD_WORK_SIMPLE_EXPORT_HEADERS.length; i++) {
            cells.get(0, i).putValue(FIELD_WORK_SIMPLE_EXPORT_HEADERS[i]);
    ...
    private void writeFieldWorkSimpleExportRow(Cells cells, int rowIndex, FieldWork fw, String allowanceNames,
                                               Style textStyle, Style dateStyle, Style dateTimeStyle, Style amountStyle) {
        final String reviewStatus = convertFieldWorkReviewStatus(fw.getReviewResult());
        final String reviewResult = StringUtils.defaultIfBlank(fw.getReviewReason(), reviewStatus);
    ...
    private void applyFieldWorkSimpleExportColumnWidth(Cells cells) {
        int[] widths = {12, 12, 14, 22, 14, 18, 24, 30, 12, 14, 12, 24, 20};
        for (int i = 0; i < widths.length; i++) {
            cells.setColumnWidth(i, widths[i]);
        }
    }
