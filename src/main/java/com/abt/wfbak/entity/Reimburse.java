...
public class Reimburse extends AuditInfo {
    ...
    private String terminateId = "System";
    @Column(columnDefinition="VARCHAR(128)")
    private String terminateName = "System";
     ...
     * 驳回流程
     */
    public void taskReject(String terminateId, String terminateName) {
        finish(STATE_REJECT, DELETE_REASON_REJECT, terminateId, terminateName);
    }

    /**
     * 流程撤销
     */
    public void processRevoke(String terminateId, String terminateName) {
        finish(STATE_CANCEL, DELETE_REASON_REVOKE, terminateId, terminateName);
    }


    public void userDeleteProcess(String terminateId, String terminateName) {
        finish(STATE_DELETE, DELETE_REASON_DELETE, terminateId, terminateName);
    }
     ...
     * @param terminateId 流程终止用户Id
     * @param terminateName 流程终止用户Name
     */
    public void finish(int state, String deleteReason, String terminateId, String terminateName) {
        this.isFinished = true;
        this.endTime = LocalDateTime.now();
        this.state = state;
        this.deleteReason = deleteReason;
        if (StringUtils.isNotBlank(terminateId)) {
            this.terminateId = terminateId;
            this.terminateName = terminateName;
        }
    }
    ...
    public void autoFinish() {
        ...
        this.endTime = LocalDateTime.now();
        this.state = STATE_PASS;
        finish(STATE_PASS, DELETE_REASON_AUTO, null, null);
    }
