...
- 创建主从表结构时，从表关联主表的外键字段必须建立合理索引，例如 `detail.main_id`
- 索引字段和顺序必须结合实际过滤、关联和排序条件设计，禁止仅创建外键但不评估关联查询索引
- 查询条件包含从表字段、查询结果以主表为主时，优先使用 `EXISTS` 子查询判断从表记录是否存在
- 禁止仅为从表查询条件直接关联并展开全部从表数据，避免主表重复和无效数据传输
- 主表分页并需要返回一对多从表数据时，优先使用两次查询：第一次只分页查询主表，第二次按当前页主表 ID 批量查询从表
- 一对多、`ManyToMany` 集合禁止在分页查询中直接使用 `JOIN FETCH`，避免 Hibernate 内存分页
- 第二次批量查询必须控制 `IN` 参数数量，并兼容 SQL Server 单条语句参数数量限制
- 从表只用于筛选时，`EXISTS` 与返回的从表数据必须解耦；返回完整从表集合时，不得只返回命中过滤条件的从表记录
- `ManyToOne`、`OneToOne` 等不会放大主表行数的单值关联，可根据实际需要使用 `JOIN FETCH`
