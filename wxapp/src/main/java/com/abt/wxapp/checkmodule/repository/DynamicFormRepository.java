...
public interface DynamicFormRepository extends JpaRepository<DynamicForm, Long> {
     ...
     */
    Optional<DynamicForm> findFirstByCheckModuleIdOrderByIdDesc(String checkModuleId);

    /**
     * 每个检测项目（cm_id）仅一条：该记录下 id 最大，即最新一版
     */
    @Query("""
            select d from DynamicForm d
            where d.id = (
                select max(d2.id) from DynamicForm d2 where d2.checkModuleId = d.checkModuleId
            )
            """)
    List<DynamicForm> findAllLatestPerCheckModule();
}
